...
export const DataProvider: React.FC<{ children?: ReactNode}> = ({ children }) => {
    ...
    useEffect(() => {
        const loadData = () => {
            ...
            if (!profile) {
                ...
                const defaultProfile = new Profile('사용자', 0, []);
    ...
    const handleUpdateProfile = (newProfileData: Profile) => {
    ...
    const handleAddDietLog = (newDietLog: DietLog) => {
        ...
        dietLogData.dietLogs.forEach((logs, date) => {
            newDietLogData.dietLogs.set(date, [...logs]);
        });
        newDietLogData.addDietLog(newDietLog);
